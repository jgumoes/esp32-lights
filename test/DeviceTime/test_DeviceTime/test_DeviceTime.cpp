...
namespace DeviceTimeTestObjects
{
  struct TestObjectsStruct{
    ...
    std::shared_ptr<OnboardTimestamp> onboardTime;
    std::shared_ptr<DeviceTimeClass> deviceTime;
  };

  /**
   * @brief construct device time testobjects from a configs struct
   * 
   * @param initialConfigs 
   * @return TestObjectsStruct 
   */
  TestObjectsStruct deviceTimeFactory(TimeConfigsStruct initialConfigs){
    TestObjectsStruct objects;
    ...
    objects.storageAdapter = std::make_shared<FakeStorageAdapter>(initialConfigMap, true, "deviceTimeFactory");
    objects.configClass = std::make_shared<ConfigStorageClass>(objects.storageAdapter);

    objects.deviceTime = std::make_shared<DeviceTimeClass>(objects.configClass);

    objects.configClass->loadAllConfigs();

    return objects;
  }

  /**
   * @brief construct device time testobjects from a TestTimeParamsStruct
   * 
   * @param initTimeParams 
   * @return TestObjectsStruct 
   */
  TestObjectsStruct deviceTimeFactory(TestTimeParamsStruct initTimeParams = testArray.at(0)){
    TestObjectsStruct objects = deviceTimeFactory(
      TimeConfigsStruct{
        .timezone = initTimeParams.timezone,
        .DST = initTimeParams.DST
      }
    );

    objects.onboardTime->setTimestamp_S(initTimeParams.localTimestamp - initTimeParams.DST - initTimeParams.timezone);
    return objects;
  }
} // namespace DeviceTimeTestObjects
...
void makeSureBuildTimeIsAsExpected(void){
  using namespace DeviceTimeTestObjects;
...
void getTimestampRounding(void){
  ...
  TestObjectsStruct testObjects = deviceTimeFactory(time1);
  std::shared_ptr<DeviceTimeClass> deviceTime = testObjects.deviceTime;
...
void setLocalTimestamp2000(void){
  ...
  // setup
  TestObjectsStruct testObjects = deviceTimeFactory();
  std::shared_ptr<DeviceTimeClass> deviceTime = testObjects.deviceTime;
  ...
  // test setLocalTimestamp without config changes
  {
    ...
    TimeConfigsStruct configs = deviceTime->getConfigs();
  ...
  for(int i = 0; i < testArray.size(); i++)
  {
    ...
    {
      TimeConfigsStruct storedConfig;
      ConfigStructFncs::deserialize(
        testObjects.storageAdapter->getConfig(ModuleID::deviceTime),
        storedConfig
      );
      TEST_ASSERT_EQUAL(storedConfig.DST, testParams.DST);
      TEST_ASSERT_EQUAL(storedConfig.timezone, testParams.timezone);

      // note: test_ConfigManager tests that DeviceTime configs don't notify when coming from DeviceTime
    }
...
void testTimeFault(){
  ...
  TimeConfigsStruct configsStruct = {0, 0, secondsInDay};
  TestObjectsStruct testObjects = deviceTimeFactory(configsStruct);
  auto testClass = testObjects.deviceTime;
  auto onboardTime = testObjects.onboardTime;
...
void testErrorsCatching(void){
  ...
  TestObjectsStruct testObjects = deviceTimeFactory(goodTime);
  std::shared_ptr<DeviceTimeClass> testClass = testObjects.deviceTime;

  auto getConfigStruct = [testObjects](){
    TimeConfigsStruct timeConfigs;
    ConfigStructFncs::deserialize(
      testObjects.storageAdapter->getConfig(ModuleID::deviceTime),
      timeConfigs
    );
  ...
  // bad timezone
  {
    ...
    TEST_ASSERT_NOT_EQUAL(badTimezone1.timezone, getConfigStruct().timezone);
    TEST_TIME_OBSERVER_NOT_NOTIFIED(observer);
    ...
    TEST_ASSERT_NOT_EQUAL(badTimezone2.timezone, getConfigStruct().timezone);
    TEST_TIME_OBSERVER_NOT_NOTIFIED(observer);
    ...
    TEST_ASSERT_NOT_EQUAL(badTimezone3.timezone, getConfigStruct().timezone);
    TEST_TIME_OBSERVER_NOT_NOTIFIED(observer);
  }
  ...
  // bad DST
  {
    ...
    TEST_ASSERT_NOT_EQUAL(badDST1.DST, getConfigStruct().DST);
    TEST_TIME_OBSERVER_NOT_NOTIFIED(observer);
    ...
    TEST_ASSERT_NOT_EQUAL(badDST2.DST, getConfigStruct().DST);
    TEST_TIME_OBSERVER_NOT_NOTIFIED(observer);
    ...
    TEST_ASSERT_NOT_EQUAL(badDST3.DST, getConfigStruct().DST);
    TEST_TIME_OBSERVER_NOT_NOTIFIED(observer);
  }
  ...
  // bad time
  {
    ...
    TimeConfigsStruct testConfigs = getConfigStruct();
...
void test_UsefulTimeStruct(){
  ...
  const int timesPerDay = 30;
  for(int day = 1; day < 8; day++){
    for(int K = 0; K <= timesPerDay; K++){
      const uint32_t timeInDay = (K*(secondsInDay-1))/timesPerDay;
...
void testConfigChanges(){
  ...
  TestObjectsStruct testObjects = deviceTimeFactory(testParams);
  std::shared_ptr<DeviceTimeClass> deviceTime = testObjects.deviceTime;
  auto configClass = testObjects.configClass;
  ...
  ConfigStructFncs::serialize(serializedConfigs, configs);
  configClass->setConfig(serializedConfigs);
  {
  ...
  // max time between syncs change
  TEST_ASSERT_NOT_EQUAL(10*60*60, configs.maxSecondsBetweenSyncs);
  observer.getCallCountAndReset();
  configs.maxSecondsBetweenSyncs = 10*60*60;
  ConfigStructFncs::serialize(serializedConfigs, configs);
  configClass->setConfig(serializedConfigs);
  TEST_ASSERT_EQUAL(0, observer.getCallCount());
}
