...
uint64_t inline findPreviousTriggerTime(TimeEventDataStruct event, UsefulTimeStruct timeStruct){
...
EventManager::EventManager(std::shared_ptr<ModalLightsInterface> modalLights, uint64_t timestampS, std::vector<TimeEventDataStruct> eventStructs) : _modalLights(modalLights)
{
  ...
  for(TimeEventDataStruct event : eventStructs){
    ...
    if(_addEvent(timestampS - (event.eventWindow * event.isActive), event) == EventManagerErrors::success){
      if(event.isActive == true){
        uint64_t triggerTime = _events[event.eventID].nextTriggerTime;
        if(triggerTime <= timestampS
          ...
          if(currentActiveEventID != 0){
            _events[currentActiveEventID].nextTriggerTime = _findNextTriggerTime(timestampS, event.eventID);
          }
          currentActiveEventID = event.eventID;
        ...
        if(triggerTime > currentBackgroundEventTime){
          ...
          currentBackgroundEventID = event.eventID;
        }
      }
    };
  };
  if(currentActiveEventID != 0){
    ...
    _callEvent(timestampS, currentActiveEventID);
  }
...
eventError_t EventManager::addEvent(uint64_t timestampS, TimeEventDataStruct newEvent)
{
  ...
  uint64_t searchTimestamp = newEvent.isActive
    ? searchTimestamp 
    : searchTimestamp;
  eventError_t error = _addEvent(timestampS - (newEvent.isActive*newEvent.eventWindow), newEvent);
  if(error != EventManagerErrors::success){
    return error;
  }
  if(_events[newEventID].isActive == false){
    uint64_t previousTriggerTime = findPreviousTriggerTime(newEvent, getUsefulTimeStruct(timestampS));
...
{
  {
    while(timestampS >= _nextEventTime){
      if(_events[_nextEventID].isActive == false
        ...
        && timestampS <= (_nextEventTime + _events[_nextEventID].eventWindow))
      ){
